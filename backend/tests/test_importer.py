...
def test_import_and_dedupe(client, account):
    ...
    # 匯入就自動填停損：獲利單 400 美元 / (1 口 × $2) = 200 點，pnl 38.52 → 0.096 R
    assert t["planned_stop_pts"] == 200 and round(t["r_multiple"], 3) == 0.096
...
def test_be_stats(client, account):
    """保本出場自動判：只小賠（賠不到計畫風險一半，400 的一半 = 200）算保本；小賺不算"""
    files = {"file": ("t.csv", SAMPLE.read_bytes(), "text/csv")}
    client.post("/api/trades/import", data={"account_name": account["name"]}, files=files)
    ...
    client.post("/api/trades", json={**base, "pnl": -190})
    client.post("/api/trades", json={**base, "pnl": 60})
    client.post("/api/trades", json={**base, "pnl": -210})
    ex = client.get("/api/stats/performance").json()["excursion"]
