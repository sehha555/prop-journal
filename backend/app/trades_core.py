    ...
    try:
        cur = conn.execute(
            """INSERT INTO trades (account_id, external_id, contract, symbol_root, direction, size,
               ...
               planned_stop_pts, mfe_pts, mae_pts, tilt, setup, note, risk_usd, r_multiple, session)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (t.account_id, ext, t.contract.upper(), d["symbol_root"], t.direction, t.size,
             ...
             t.planned_stop_pts, t.mfe_pts, t.mae_pts, int(t.tilt), t.setup, t.note, d["risk_usd"], d["r_multiple"], d["session"]),
        )
